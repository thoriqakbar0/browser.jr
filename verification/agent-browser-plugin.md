...
| ABPLUGIN-07 | P2 | pipe | Native startup and shutdown failures remain bounded. | Use missing, no-ready, signal-ignoring, bind-failure, and idle-client fixtures. | Start a session or relay, then trigger each failure. | The plugin returns one JSON failure or closes within the stated bound. | fail: relay startup and shutdown can remain alive, BJR-014, 2026-09-02 |
| ABPLUGIN-08 | P1 | pipe | Repository state blocks npm publication before release decisions are complete. | Use the current `package.json` without npm credentials. | Run `npm publish --dry-run --ignore-scripts`. | The command fails before the registry publication step. | fail: public dry run succeeds, BJR-015, 2026-09-02 |
| ABPLUGIN-09 | P2 | pipe | Relay port validation rejects trailing text. | Use a missing controlled binary. | Start `serve` with `--port 54321junk`. | The plugin returns an invalid-port response before native startup. | fail: native startup is attempted, BJR-016, 2026-09-02 |
