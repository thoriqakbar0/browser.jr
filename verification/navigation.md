...
| NAV-07 | P1 | pipe | Native link Enter shares transactional navigation ([Finish](../loading/navigation.md#finish)). | Serve a link and controlled destination. | Activate through locator Enter, go back, then activate through stored focus. | Both navigations record history and clear old references. A failed load preserves state. | partial: package and compiled-process tests passed, 2026-08-31 |
| NAV-08 | P1 | package | Link click blocks unsupported motion before navigation ([Begin running](../loading/navigation.md#begin-running)). | Give a same-context link an inline transition declaration. | Click it through a strict locator, then read the current URL. | The stable check blocks. The source URL remains current. | partial: package test passed, 2026-09-01 |
